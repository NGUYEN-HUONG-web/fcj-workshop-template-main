...
Create a local `.env` file from the project example and provide values for database credentials, Redis, MinIO, AI model configuration, application URL, and storage settings. Never commit `.env` to Git.
 Repository structure and sanitized `.env.example`
 ```bash
# ==================== 基础配置 ====================
LOG_DEPTH=3
# 默认用户密码（用户名为 root），每次重启会自动更新。
DEFAULT_ROOT_PSW=CHANGE_ME_ROOT_PASSWORD
# 数据库最大连接数
DB_MAX_LINK=5
# 自动同步索引
SYNC_INDEX=true
...
FILE_TOKEN_KEY=
# 密钥加密 key
AES256_SECRET_KEY=CHANGE_ME_AES256_SECRET
# Invoke 反向调用 JWT 密钥，至少 32 位
INVOKE_TOKEN_SECRET=CHANGE_ME_WITH_AT_LEAST_32_RANDOM_CHARACTERS
# root key（最高权限）
ROOT_KEY=CHANGE_ME_ROOT_KEY
...
PLUGIN_TOKEN=CHANGE_ME_PLUGIN_TOKEN

# 代码沙箱服务
CODE_SANDBOX_URL=http://localhost:3002
CODE_SANDBOX_TOKEN=CHANGE_ME_CODE_SANDBOX_TOKEN

# AI Proxy API
AIPROXY_API_ENDPOINT=http://localhost:3010
AIPROXY_API_TOKEN=CHANGE_ME_AIPROXY_TOKEN
...
AGENT_SANDBOX_OPENSANDBOX_API_KEY=CHANGE_ME_OPENSANDBOX_API_KEY
...
AGENT_SANDBOX_OPENSANDBOX_VOLUME_MANAGER_TOKEN=CHANGE_ME_VOLUME_MANAGER_TOKEN
...
AGENT_SANDBOX_PROXY_SECRET=CHANGE_ME_WITH_AT_LEAST_32_RANDOM_CHARACTERS
...
STORAGE_ACCESS_KEY_ID=CHANGE_ME_STORAGE_ACCESS_KEY
STORAGE_SECRET_ACCESS_KEY=CHANGE_ME_STORAGE_SECRET_KEY
...
MONGODB_URI=mongodb://CHANGE_ME_USERNAME:CHANGE_ME_PASSWORD@localhost:27017/fastgpt?authSource=admin&directConnection=true
# 日志库
MONGODB_LOG_URI=
...
# PG 向量库连接参数
PG_URL=postgresql://CHANGE_ME_USERNAME:CHANGE_ME_PASSWORD@localhost:5432/postgres
# OceanBase 向量库连接参数
# OCEANBASE_URL=
# SeekDB 向量库连接参数
# SEEKDB_URL=
...
# AWS Learning Assistant 真实 RAG 集成（公开资源 ID，不要填写 API Key）
# NEXT_PUBLIC_AWS_LEARNING_APP_ID=FastGPT 应用/工作流 ID
# NEXT_PUBLIC_AWS_LEARNING_DATASET_ID=FastGPT Knowledge Base ID
